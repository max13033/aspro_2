...
window.JCCatalogSection.OnPriceFromCatalogSection = function()
{
	if($('.js-property-price-from-Y').length>0){
        $('.item_block').each(function(){
            if($(this).hasClass('js-property-price-from-Y')){
                console.log('js-property-price-from-')
                $(this).find('.values_wrapper .js-price-from').remove()
                $(this).find('.values_wrapper').prepend('<span class="js-price-from">От </span>')
            }
        })
	}
};
