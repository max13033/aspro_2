...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		for(var PID in result.ITEMS)
		{
			...
			{
                /*var item__ = result.ITEMS[PID]
                if(
                    (
                        item__.VALUES.length>0
                        ||
                        !item__.PRICE
                    )
                    &&
                    typeof item__.VALUES['MAX'] != 'object'
                    &&
                    item__.NAME == 'Производитель'
                ){
                    var sortable = new Array();
                    var sortableO = new Object();

                    for (var vehicle in item__.VALUES) {
                        sortable.push([vehicle, item__.VALUES[vehicle]]);
                    }

                    sortable.sort(function(a, b) {
                        return Number(a[1].ELEMENT_COUNT) - Number(b[1].ELEMENT_COUNT);
                    });

                    var xArray = $.extend([], sortable.reverse());

                    xArray.forEach(function (sort_arr, i) {
                        console.log(sort_arr[0]+' =>>>> '+sort_arr[1].ELEMENT_COUNT)
                        sortableO[i] = sort_arr[1];
                    })

                    result.ITEMS[PID].VALUES = sortableO

                    console.log({
						x: 'НЕ ПРОПУСКАЮ ',
						array_sort: xArray,
						array_val_sort: sortableO,
						array_val_notsort: result.ITEMS[PID].VALUES,
					})
                }*/
